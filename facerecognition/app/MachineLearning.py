...
emotion_recognition_model = pickle.load(open(os.path.join(STATIC_DIR,'models/machinelearning_face_emotion_identity.pkl'),mode='rb'))
    ...
    if len(detections) > 0:
        for i , confidence in enumerate(detections[0,0,:,2]):
            if confidence > 0.5:
                ...
                cv2.imwrite(os.path.join(settings.MEDIA_ROOT,'mi_output/process.jpg'),Image)
                cv2.imwrite(os.path.join(settings.MEDIA_ROOT,'mi_output/roi_{}.jpg'.format(count)),face_roi)
